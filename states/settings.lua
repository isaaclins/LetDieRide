...
function SettingsState:drawMainView(W, H)
	...
	self._back_hovered =
		UI.drawButton("BACK", btn_start_x, btn_row_y, btn_w, btn_h, { font = Fonts.get(18), color = UI.colors.blue })

	self._keybinds_hovered = UI.drawButton(
		"KEYS",
		btn_start_x + btn_w + btn_gap,
		btn_row_y,
		btn_w,
		btn_h,
...
function SettingsState:keypressed(key)
	...
	if key == "up" then
		...
		if settings_focus < 1 then
			settings_focus = item_count
		end
		return nil
	elseif key == "down" then
		settings_focus = settings_focus + 1
		if settings_focus > item_count then
			settings_focus = 1
		end
		return nil
