	...
	elseif sub_state == "rolling" then
		...
		if all_done and not player:anyDiceRolling() then
			...
			if Tutorial:isActive() then
				Tutorial:notifySubState("choosing")
			end
		end
...
function Round:updatePreview(player)
	if not player or #player.dice_pool == 0 then
		return
	end
	local values = player:getDiceValues()
	local context = {
...
function Round:drawScorePreview(player, W, H)
	if sub_state ~= "choosing" or #preview_combo == 0 then
		return
	end
	if preview_panel_anim.alpha < 0.01 then
		return
	end
	...
	love.graphics.setColor(
		UI.colors.text_dim[1],
		UI.colors.text_dim[2],
		UI.colors.text_dim[3],
		preview_panel_anim.alpha
	)
	...
	if has_mult then
		...
		love.graphics.printf(
			"Item mult: x" .. string.format("%.1f", 1 + preview_mult_bonus),
			panel_x + 12,
			ly,
			panel_w - 24,
			"left"
	...
	if has_glass then
		...
		love.graphics.printf(
			"Glass: x" .. string.format("%.2g", preview_score_mult),
			panel_x + 12,
			ly,
			panel_w - 24,
			"left"
...
function Round:drawBulkWildButton(player, W, H)
	...
	love.graphics.setColor(1, 1, 1, preview_panel_anim.alpha)
	self._bulk_wild_hovered = UI.drawButton("Set All Wild (" .. wild_total .. ")", btn_x, btn_y, btn_w, btn_h, {
		font = Fonts.get(14),
...
function getDiceLayout(count, W, H)
	...
	local gap = math.min(
		16,
		math.max(8, math.floor((avail_w - die_size * math.min(count, 6)) / math.max(math.min(count, 6) - 1, 1)))
	)
...
function Round:drawDice(player, W, H)
	...
	for i, die in ipairs(player.dice_pool) do
		...
		if (da.echo_flash or 0) > 0 then
			...
			UI.roundRect(
				"fill",
				draw_x - ring_expand / 2,
				draw_y - ring_expand / 2,
				s + ring_expand,
				s + ring_expand,
				s * 0.15 + ring_expand * 0.3
			)
...
function Round:drawHandReference(player, W, H)
	if hand_ref_anim.alpha < 0.01 then
		return
	end
...
function Round:drawActions(player, W, H)
	...
	self._reroll_hovered =
		UI.drawButton("REROLL (" .. player.rerolls_remaining .. ")", center_x - btn_w - 20, btn_y, btn_w, btn_h, {
			font = Fonts.get(20),
	...
	local hint = kn(getKeybind("select_next"))
		.. " Select  |  "
		.. kn(getKeybind("move_left"))
		.. "/"
		.. kn(getKeybind("move_right"))
		.. " Move  |  "
		.. kn(getKeybind("toggle_lock"))
		.. " Lock  |  "
		.. kn(getKeybind("reroll"))
		.. " Reroll  |  "
		.. kn(getKeybind("score"))
		.. " Score  |  "
		.. kn(getKeybind("sort_cycle"))
		.. " Sort  |  "
		.. kn(getKeybind("show_tooltip"))
		.. " Info"
...
function Round:drawScoring(player, W, H)
	...
	local bonus_section_h = (round_bonus > 0 and 16 or 0)
		+ (round_mult_bonus > 0 and 16 or 0)
		+ (round_score_mult > 1 and 16 or 0)
	local panel_h
	if won and bd_count > 0 then
		panel_h = 14 + hand_section_h + bonus_section_h + 60 + 28 + bd_count * 24 + 38 + (has_abilities and 30 or 0)
	else
		panel_h = 14 + hand_section_h + bonus_section_h + 60 + 28 + (has_abilities and 30 or 0) + 40
	end
	...
	if round_bonus > 0 then
		...
		love.graphics.printf(
			"+ " .. UI.abbreviate(round_bonus) .. " bonus",
			px + pad_x,
			ly,
			panel_w - pad_x * 2,
			"left"
	...
	if round_mult_bonus > 0 then
		local mb_str = (1 + round_mult_bonus) >= 1e3 and UI.abbreviate(1 + round_mult_bonus)
			or string.format("%.1f", 1 + round_mult_bonus)
		love.graphics.setColor(UI.colors.green[1], UI.colors.green[2], UI.colors.green[3], 0.8)
