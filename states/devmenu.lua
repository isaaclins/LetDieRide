...
local tabs = { "player", "items", "dice", "bosses", "hands" }
local tab_labels = { player = "Player", items = "Items", dice = "Dice", bosses = "Bosses", hands = "Hands" }
...
local function drawItemsTab(px, py, pw, ph)
    ...
    for idx, item in ipairs(all_items_ref) do
        ...
        for _, pi in ipairs(draft.items) do
            if pi.name == item.name then owned = true; break end
        end
...
local function drawDiceTab(px, py, pw, ph)
    ...
    for i, die in ipairs(draft.dice_pool) do
        ...
        if die.color == "blue" then dot_color = UI.colors.die_blue
        elseif die.color == "green" then dot_color = UI.colors.die_green
        elseif die.color == "red" then dot_color = UI.colors.die_red end

        UI.drawDie(dx, ly, die_size, die.value, dot_color, nil, false, false, die.glow_color)

        love.graphics.setFont(small_font)
        UI.setColor(UI.colors.text_dim)
        love.graphics.print(die.name:sub(1, 8), dx, ly + die_size + 2)
    ...
    if col > 0 then ly = ly + die_size + 22 end
    ...
    for idx, die in ipairs(all_dice_ref) do
        ...
        smallBtn(die.name:sub(1, 12), bx, ly, add_btn_w, add_btn_h, UI.colors.blue, UI.colors.blue_hover)
...
function DevMenu:draw()
    ...
    local start_hovered = UI.drawButton(
        "START DEBUG GAME", start_btn_x, start_btn_y, start_btn_w, start_btn_h,
...
function DevMenu:handleButton(id)
    ...
    if id:find("^item_") then
        ...
        if idx and all_items_ref[idx] then
            ...
            for i, pi in ipairs(draft.items) do
                if pi.name == item.name then found_idx = i; break end
            end
            if found_idx then
                table.remove(draft.items, found_idx)
                Toast.show("Removed " .. item.name, "error")
            else
                table.insert(draft.items, Item:new({
                    name = item.name,
    ...
    if id:find("^dice_add_") then
        ...
        if idx and all_dice_ref[idx] then
            ...
            _btn_regions = {}
            Toast.show("Added " .. new_die.name, "success")
        end
        return nil
    end
...
function DevMenu:keypressed(key)
    ...
    for i, t in ipairs(tabs) do
        if t == current_tab then tab_idx = i; break end
    end
