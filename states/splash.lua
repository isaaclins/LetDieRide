...
function Splash:draw()
	...
	if has_save then
		...
		if splash_focus == focus_idx then
			UI.drawFocusRect(btn_x, btn_y + ba.y_off, btn_w, btn_h)
		end
		btn_y = btn_y + 68
		btn_idx = btn_idx + 1
	else
	...
	if splash_focus == focus_idx then
		UI.drawFocusRect(btn_x, btn_y + 68 + ba.y_off, btn_w, btn_h)
	end
	btn_idx = btn_idx + 1
	...
	if splash_focus == focus_idx then
		UI.drawFocusRect(btn_x, btn_y + 136 + ba.y_off, btn_w, btn_h)
	end
	btn_idx = btn_idx + 1
	...
	if splash_focus == focus_idx then
		UI.drawFocusRect(btn_x, btn_y + 204 + ba.y_off, btn_w, btn_h)
	end
...
function Splash:keypressed(key)
	if splash_btn_count > 0 then
		if key == "up" then
			...
			if splash_focus < 1 then
				splash_focus = splash_btn_count
			end
			return nil
		elseif key == "down" then
			splash_focus = splash_focus + 1
			if splash_focus > splash_btn_count then
				splash_focus = 1
			end
			return nil
		end
